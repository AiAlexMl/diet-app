...
- **Account layer (phase 1, live)**: header account icon (injected by `supabase-client.js`; anonymous → login modal, connected → account modal with day history from `day_logs` + favorites, logout, delete-account). **Favorites heart** on the menu header snapshots the current day (`localStorage['shapeat-favorites']`, cap 30; re-click same day updates, never duplicates) and mirrors to the cloud `favorites` table when connected. Read-only day viewing reuses `dayHtml(day, {readOnly: true})` (ui.js).
