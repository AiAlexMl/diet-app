...
- **Instagram posts (workflow)**: `internal/POST-LOG.md` is the **canonical log** of everything published/planned. **Before creating any new IG post / reel / story, read it first** — check what already shipped and which angles are taken (the "זוויות תפוסות" line) — and only then decide on content, so posts don't repeat. After building one, log it there (full date, status, actual caption). Reel/post assets + sources live under **`internal/posts/<name>/`** (private repo — GitHub Pages serves this public repo, so anything committed here is live on shapeat.co.il and indexable, which would publish a reel before it goes up on Instagram). The **only** exception is `brand/posts/coach-demo-video/` + its `record-coach-demo.mjs`, which stay public because `coaches.html` embeds the video. Reels are built with HyperFrames (scaffold lives outside the repo, see the asset's `README.md`).
...
- **Macro accuracy**: best-of-5 meal builds + 3-stage `reconcile()`, 1.6 g/kg protein floor, `S.menuWarning` on infeasible low targets. **Fiber floor** 14 g/1000 kcal (`S.fibG`) steered from-the-source in the `buildMealBest` scoring (floor + far-band ceiling; no reconcile stage). **Full mechanics + measured accuracy: `.claude/rules/algorithm.md`**
...
- **Account layer (phase 1, live)**: header account icon (injected by `supabase-client.js`; anonymous → login modal, connected → account modal, a near-fullscreen sheet whose main view is content-only, tabbed: **"התקדמות 📈" (default — the weight graph)** + day history from `day_logs` + favorites. Account management (email, logout, delete-account) lives one level deeper behind a quiet ⚙ gear → `account-settings` sub-view, so destructive/rare actions aren't in the daily content view). **Favorites heart** on the menu header snapshots the current day (`localStorage['shapeat-favorites']`, cap 30, one per build (`buildId`) so multiple menus per date are kept; re-click toggles off with an undo toast) and mirrors to the cloud `favorites` table when connected. Read-only day viewing reuses `dayHtml(day, {readOnly: true})` (ui.js).
- **Weight tracking v1 (live)**: account-gated progress graph, an **additive layer in `supabase-client.js`** (like the account layer — near-zero touch to ui/app). Local-first cache `localStorage['shapeat-weights']` mirrored to cloud `weight_logs` (RLS, migration `003_weight_logs.sql`) via the same push/pull outbox as favorites. A slot after the daily-summary card shows one of three states: anonymous → "התחילו לעקוב · חינם" teaser (opens login); connected+no-data → **seed-confirm** empty-state pre-filled with the weight entered at menu-build (`S.weight`, one-tap "כן, התחל"); connected+data → mini widget (last kg + sparkline + trend + weigh-in). Weekly nudge (weeks-since-last ≥ 1, on app-open, no push). Weight-line-only kg graph (vanilla SVG, numeric-only innerHTML; RTL number labels wrapped `dir="ltr"`). **Safety:** input clamped 30–300 kg; neutral/no loss-celebration; **once-per-episode underweight referral** when BMI < 18.5, suppressed for a bulk goal that isn't losing, fired for a bulk goal that is (see `underweightReferral`). **Deferred (ROADMAP):** weight→target adjustment (BMI hard-stops must re-run on updated weight then) and coach weight-trend (needs privacy.html + consent-screen updates). Full frozen spec: `internal/ROADMAP.md` "מעקב משקל v1".
